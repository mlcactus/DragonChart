...
DChart.Polar = DChart.getCore().__extends({
    ...
    Draw: function (_data, ops) {
        ...
        var polarRadius = options.radius && options.radius < radiusInfo.maxRadius ? options.radius : radiusInfo.maxRadius;
        inner.coordinates.draw = radiusInfo.coordinate;
        inner.coordinates.polar = { radius: polarRadius, centerX: radiusInfo.centerX, centerY: radiusInfo.centerY };
        ...
        var mouseEvents = function () {
            ...
            inner.canvas.onmousemove = function (e) {
                ...
                if (inner.tempData.currentMouseShape != veryShape) {
                    ...
                    inner._clearDrawable(radiusInfo.coordinate);
                    drawOuterLabels();
                    for (var i = 0, shape; shape = inner.shapes.cemicircles[i]; i++) {
                        if (shape.isHovered) {
