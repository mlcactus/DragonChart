...
DChart.Area._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var redraw = function () {
        if (axisData.multiple) {
            ...
            inner.DrawFigures.createCloseFigure(points, redrawRecord.fillcolor, 0, null, smoothline, upturnAxis);
            var linepoints = points.slice(1, points.length - 1);
            drawlineFunction(linepoints, redrawRecord.linewidth, redrawRecord.linecolor, upturnAxis);
            for (var j = 0; j < linepoints.length; j++) {
