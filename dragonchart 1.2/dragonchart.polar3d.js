if (!window.DChart) {
    ...
    DChart.Const.Skins.BlackAndWhite.Polar3D = {
...
DChart.Polar3D = DChart.getCore().__extends({ GraphType: 'Polar3D' });
DChart.Polar3D._spreadSkin = function (newOps, skin) {
...
DChart.Polar3D._getDefaultOptions = function (originalCommonOptions) {
...
DChart.Polar3D._getCheckOptions = function () {
    return {
...
DChart.Polar3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var polarRadius = !options.radius || !DChart.Methods.IsNumber(options.radius) ? radiusInfo.maxRadius : options.radius;
    ...
    var drawPart = function (drawtype, radius, angleMin, angleMax, color, mouseon, darksidecolor, data, pieshape) {
        ...
        switch (drawtype) {
            ...
            default:
                ...
                if (_linewidth > 0) {
                    if (DChart.Methods.JudgeBetweenAngle(0, Math.PI, angleMin)) {
                    ...
                    if (DChart.Methods.JudgeBetweenAngle(0, Math.PI, angleMax)) {
        ...
        if (data && ops.show && typeof ops.content == 'function') {
            ...
            var floatright = DChart.Methods.JudgeBetweenAngle(-Math.PI * 0.5, Math.PI * 0.5, computeinfo.midAngle);
            var floattop = DChart.Methods.JudgeBetweenAngle(-Math.PI, 0, computeinfo.midAngle);
    ...
    var drawScales = function (recordStaff) {
        ...
        for (var val = scaleData.minvalue + scaleData.interval; val <= scaleData.maxvalue; val += scaleData.interval) {
            ...
            var text = content(DChart.Methods.FormatNumber(val));
    ...
    var mouseEvents = function () {
        var fixShape = function (x, y) {
            ...
            for (var i = 0, shape; shape = shapes.cemicircles[i]; i++) {
                ...
                var currentAngle = DChart.Methods.GetCurrentAngle(x, y, computeinfo.centerX, computeinfo.centerY);
                var withinPie = (Math.pow(x - computeinfo.centerX, 2) + Math.pow(y - computeinfo.centerY, 2) <= Math.pow(shape.radius, 2)) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentAngle);
            ...
            if (!veryShape) {
                for (var i = shapes.cemicircles.length - 1; i >= 0; i--) {
                    ...
                    var withinDarkPie = (Math.pow(x - computeinfo.darkCenterX, 2) + Math.pow(y - computeinfo.darkCenterY, 2) <= Math.pow(shape.radius, 2)) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentDarkAngle);
                    if (withinDarkPie) {
                        veryShape = shape; break;
                    }
                }
