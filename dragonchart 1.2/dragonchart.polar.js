...
DChart.Polar = DChart.getCore().__extends({ GraphType: 'Polar' });
DChart.Polar._spreadSkin = function (newOps, skin) {
...
DChart.Polar._getDefaultOptions = function (originalCommonOptions) {
...
DChart.Polar._getCheckOptions = function () {
    return {
...
DChart.Polar._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var drawSegments = function (animationDecimal, percentAnimComplete) {
        ...
        for (var i = 0, item; item = innerData[i]; i++) {
            ...
            if (percentAnimComplete >= 1) {
                ...
                inner.coordinates.polar[graphicID].cemicircles.push({ index: i, percent: percent, radius: radiusPercent, angleMin: cumulativeAngle, angleMax: cumulativeAngle + segmentAngle, color: color });
            }
            else {
