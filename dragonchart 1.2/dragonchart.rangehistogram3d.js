if (!window.DChart) {
    ...
    DChart.Const.Skins.BlackAndWhite.RangeHistogram3D = {
        SmallLabelColor: null,
        BigLabelColor: null
    };
}
DChart.RangeHistogram3D = DChart.getCore().__extends({ GraphType: 'RangeHistogram3D' });
DChart.RangeHistogram3D._spreadSkin = function (newOps, skin) {
    newOps.smallLabel = {};
    newOps.smallLabel.color = skin.SmallLabelColor || null;
    newOps.bigLabel = {};
    newOps.bigLabel.color = skin.BigLabelColor || null;
};
DChart.RangeHistogram3D._getDefaultOptions = function (originalCommonOptions) {
...
DChart.RangeHistogram3D._getCheckOptions = function () {
    return {
...
DChart.RangeHistogram3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var drawPart = function (shape, color, mouseon, _shadow, drawgraphic, drawtext) {
        ...
        if (drawgraphic) {
            inner.DrawFigures.create3DHistogram(_left, _top, shape.width, shape.height, _color, 0, axisSize.sightangle, axisSize.stagewidth, null, mouseon ? color : shape.topcolor || DChart.Methods.getDarkenColor(_color, 0.7), mouseon ? color : shape.rightcolor || DChart.Methods.getDarkenColor(_color, 0.8), _shadow);
        }
        if (drawtext) {
            ...
            if (ops.show) {
                ...
                if (minContent) {
                    var fontsize = shape.data.smallfontsize || ops.fontsize || DChart.Methods.CapValue(height, 18, 11);
            ...
            if (ops.show) {
                ...
                if (maxContent) {
                    var fontsize = shape.data.bigfontsize || ops.fontsize || DChart.Methods.CapValue(height, 18, 11);
