if (!window.DChart) {
    ...
    DChart.Const.Skins.BlackAndWhite.Ring3D = {
...
DChart.Ring3D = DChart.getCore().__extends({ GraphType: 'Ring3D' });
DChart.Ring3D._spreadSkin = function (newOps, skin) {
...
DChart.Ring3D._getDefaultOptions = function (originalCommonOptions) {
...
DChart.Ring3D._getCheckOptions = function () {
    return {
...
DChart.Ring3D._drawgraphic = function (inner, graphicID, innerData, options) {
        ...
        var fixShape = function (x, y) {
            ...
            if (!veryShape) {
                for (var i = shapes.cemicircles.length - 1; i >= 0; i--) {
                    ...
                    var withinDarkPie = distance2 >= Math.pow(pieInnerRadius, 2) && distance2 <= Math.pow(pieOuterRadius, 2) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentDarkAngle);
                    if (withinDarkPie) {
                        veryShape = shape; break;
                    }
                }
