if (!window.DChart) {
    ...
    DChart.Const.Skins.BlackAndWhite.Pie3D = {
...
DChart.Pie3D = DChart.getCore().__extends({ GraphType: 'Pie3D' });
DChart.Pie3D._spreadSkin = function (newOps, skin) {
...
DChart.Pie3D._getDefaultOptions = function (originalCommonOptions) {
...
DChart.Pie3D._getCheckOptions = function () {
    return {
...
DChart.Pie3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var mouseEvents = function () {
        var fixShape = function (x, y) {
            ...
            for (var i = 0, shape; shape = shapes.cemicircles[i]; i++) {
                ...
                var withinPie = (Math.pow(x - computeinfo.centerX, 2) + Math.pow(y - computeinfo.centerY, 2) <= Math.pow(pieRadius, 2)) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentAngle);
            ...
            if (!veryShape) {
                for (var i = shapes.cemicircles.length - 1; i >= 0; i--) {
                    ...
                    var withinDarkPie = (Math.pow(x - computeinfo.darkCenterX, 2) + Math.pow(y - computeinfo.darkCenterY, 2) <= Math.pow(pieRadius, 2)) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentDarkAngle);
                    if (withinDarkPie) {
                        veryShape = shape; break;
                    }
                }
