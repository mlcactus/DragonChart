...
DChart.Methods = {
    ...
    getDarkenColor: function (color, proportion) {
        ...
        var rgba = DChart.Methods.getRGBA(color);
        if (rgba) {
...
DChart.getCore = function () {
    var core = function (_targetdiv, _language) {
        ...
        inner.ChangeLanguage = function (language) {
            inner.Language = DChart.Const.Language[language] != undefined ? language : 'CN';
            inner._messages = DChart.Const.Language[inner.Language];
        };
        ...
        inner._initial = function () {
            ...
            inner.ID = 'DChart_' + DChart.Methods.GetRandomString();
            inner.Initial();
        };
        inner._computeBasic = function (_ops) {
            ...
            var width = DChart.Methods.CapValue(ops.Width || defaultWidth, inner.parentdiv.clientWidth, 0);
            var height = DChart.Methods.CapValue(ops.Height || defaultHeight, inner.parentdiv.clientHeight, 0);
        ...
        inner.SetBasicOptions = function (ops) {
            ...
            if (calculatedBasic && DChart.Methods.ObjectHaveSameValues(calculatedBasic, _computedNew, ['offleft', 'offright', 'offtop', 'offbottom', 'height', 'width'])) {
                return;
            }
            inner._configs.basicoptions = DChart.Methods.Override(inner._configs.basicoptions, ops);
            inner.Initial();
            return inner;
        };
        ...
        inner.SetDefaultOptions = function () {
            ...
            inner.innerOptions = DChart[inner.GraphType]._getDefaultOptions(inner._configs.originalCommonOptions);
            return inner;
        };
        ...
        inner.SetData = function (data) {
            ...
            if (data) {
                ...
                for (var i = 0; i < inner.innerData.length; i++) {
                    inner._configs.dataBackup[i] = DChart.Methods.DeepCopy(inner.innerData[i]);
                }
        ...
        inner._checkOptions = function () {
            ...
            var commonsets = {
                ...
                legend: [['show', 'b'], ['enablecontrol', 'b'], ['linethrouth', 'b'], ['type', 's'], ['elementtype', 's'], ['placeX', 's'], ['placeY', 's'], ['sidelength', 'n'], ['offX', 'n'], ['offY', 'n'], ['bordercolor', 'c'], ['borderwidth', 'n'], ['fontcolor', 'c'], ['fontsize', 'n'], ['fontfamily', 's']],
            ...
            var checksets = function (sets, options) {
                for (var _item in sets) {
                    var ops = _item == '__top' ? options : options[_item];
            ...
            checksets(commonsets, inner.innerOptions);
            checksets(DChart[inner.GraphType]._getCheckOptions(), inner.innerOptions);
            for (var i = 0, plugin; plugin = inner.plugins[i]; i++) {
                checksets(commonsets, plugin.pluginOptions);
                checksets(DChart[plugin.pluginType]._getCheckOptions(), plugin.pluginOptions);
            }
        };
        inner.AddPlugin = function (type, data, options, zIndex) {
            if (typeof type != 'string' || !DChart.Const.DrawAxis.__contains(type) && !DChart.Const.NotDrawAxis.__contains(type)) {
                throw new Error(inner._messages.PluginParamWrong);
            }
            if (!DChart.Const.AsPlugins.__contains(type)) {
                throw new Error('"' + type + '"' + inner._messages.PluginNotSupportedType);
            }
            if (DChart[type] == undefined) {
                throw new Error('\'' + type + '\'' + inner._messages.GraphicTypeNotQuoted);
            }
            if (arguments.length === 2 && !DChart.Methods.IsArray(arguments[1])) {
                options = arguments[1];
                data = null;
            }
            if (data) {
            ...
            if (!options) { options = DChart[type]._getDefaultOptions(inner._configs.originalCommonOptions); }
            else { options = DChart.Methods.Override(DChart[type]._getDefaultOptions(inner._configs.originalCommonOptions), options); }
            var pluginID = 'pluginID_' + DChart.Methods.GetRandomString();
            if (!DChart.Methods.IsNumber(zIndex)) {
        ...
        inner._reApplySkinOptions = function () {
            ...
            if (inner._configs.needCheckSkin) {
                ...
                var applySkin = function (backup, type) {
                    var newOps = {};
                    var skin = DChart.Const.Skins[inner._configs.skinID][type];
                    if (DChart.Const.Skins[inner._configs.skinID] && skin) { DChart[type]._spreadSkin(newOps, skin); }
                    var tmpSkinOptions = DChart.Methods.Extend(skinCommonOptions, newOps);
                    return DChart.Methods.Extend(tmpSkinOptions, backup, true);
                };
        ...
        inner.Draw = function (_data, ops) {
            ...
            if (inner.innerData.length > 0) {
                ...
                var mainTask = DChart[inner.GraphType]._drawgraphic(inner, inner.ID, inner.innerData, innerOptions);
                mainTask.zIndex = 0;
                tasks.push(mainTask);
                for (var i = 0, plugin; plugin = inner.plugins[i]; i++) {
                    ...
                    if (pluginData.length > 0) {
                        ...
                        var task = DChart[plugin.pluginType]._drawgraphic(inner, plugin.pluginID, pluginData, plugin.pluginOptions);
        ...
        inner._startDrawAndAnimation = function (tasks) {
            ...
            var animFrameAmount = (options.animation) ? 1 / DChart.Methods.CapValue(options.animationSteps, 50, 1) : 1;
            ...
            var animLoop = function () {
                ...
                if (!valids.AxisValid || inner._configs.recreateAssists) { inner._createAssists(valids); }
                ...
                if (percentAnimComplete < 1) {
                    ...
                    if (legendcoor && options.legend.enablecontrol) {
                        ...
                        var getlegend = function (e) {
                            ...
                            var index = null;
                            if (x > legendcoor.left && x < legendcoor.left + legendcoor.width && y > legendcoor.top && y < legendcoor.top + legendcoor.height) {
                                inner._configs.cursorPointer = true;
                                ...
                                for (var i = 0; i < legendcoor.points.length; i++) {
                                    ...
                                    if (computed.type == 'column') {
                                        ...
                                        if (y > _top && y < _top + _length) { index = i; break; }
                                    }
                                    else {
                                        if (x > point.left && x < point.left + computed.sidelength + computed.sidedistance + computed.maxTextLength) { index = i; break; }
                                    }
                                }
                            };
                            return index;
        ...
        inner._createTip = function (content, left, top) {
            ...
            tipBox.setAttribute('class', inner._configs.classes[tiptype]);
            tipBox.style.position = 'absolute';
            tipBox.style.left = inner.parentdiv.offsetLeft + left + inner._configs.calculatedBasic.offleft + 'px';
            tipBox.style.top = inner.parentdiv.offsetTop + top + inner._configs.calculatedBasic.offtop + 'px';
        ...
        inner._changeTip = function (tip, left, top) {
            if (left) {
                tip.style.left = inner.parentdiv.offsetLeft + left + inner._configs.calculatedBasic.offleft + 'px';
            }
            if (top) {
                tip.style.top = inner.parentdiv.offsetTop + top + inner._configs.calculatedBasic.offtop + 'px';
            }
        };
        ...
        inner._createLegend = function (valids) {
            ...
            var ops = inner.innerOptions.legend;
            var computed = inner._computeLegend();
            var colors = inner._configs.legendColors || DChart.Const.Defaults.FillColors;
            var lineelement = computed.elementtype == 'l';
            ...
            for (var i = 0, item; item = inner._configs.dataBackup[i]; i++) {
                ...
                if (lineelement) {
                    ...
                    inner.DrawFigures.createLine(left, _top, left + computed.sidelength, _top, 2, color);
                    if (showlightcolor) {
                        inner.DrawFigures.createLine(left, _top, left + computed.sidelength, _top, 2, lightcolor);
                    }
                    ...
                    if (showlightcolor) {
                        inner.DrawFigures.createPointElement(computed.elementtype, left, top, computed.sidelength, lightcolor, computed.elementtype != 'x', lightcolor, 1, computed.elementtype == 'x');
                    }
                ...
                if (showdimcolor && ops.linethrouth) {
        ...
        inner._formatAxisData = function (heapCompute) {
            if (!DChart.Const.DrawAxis.__contains(inner.GraphType)) {
        ...
        inner._computeAxis = function (valids) {
            ...
            var minY = canvasBorderWidth + titleHeight + (legendSize && legendSize.occupyTop > 0 ? legendSize.occupyTop : xAxisHeight / 10);
            if (axis3dconfigs) {
                minX += axis3dconfigs.sinsightangle * axis3dconfigs.stagewidth;
                maxY -= axis3dconfigs.cossightangle * axis3dconfigs.stagewidth;
            }

            var axisValueCut = (upturnAxis ? maxX - minX : maxY - minY) / axisData.vScalecount;
            ...
            inner._configs.axisSize = axis3dconfigs ? DChart.Methods.Extend(axisconfigs, axis3dconfigs) : axisconfigs;
        ...
        inner._createAxis = function (valids) {
            ...
            var draw3DAxis = function () {
                ...
                var marbletopcolor = options.axis3d.marbletopcolor || DChart.Methods.getDarkenColor(marblefacecolor, 0.7);
                var marblerightcolor = options.axis3d.marblerightcolor || DChart.Methods.getDarkenColor(marblefacecolor, 0.8);
        ...
        inner.DrawFigures.createRingReflection = function (computeinfo, fillcolor, linewidth, linecolor, preventinnerLine, preventouterLine) {
            ...
            if (linewidth > 0) {
                if (DChart.Methods.JudgeBetweenAngle(0, Math.PI, computeinfo.angleMin) && !preventouterLine) {
                ...
                if (DChart.Methods.JudgeBetweenAngle(Math.PI, Math.PI * 2, computeinfo.angleMin) && !preventinnerLine) {
                ...
                if (DChart.Methods.JudgeBetweenAngle(0, Math.PI, computeinfo.angleMax) && !preventouterLine) {
                ...
                if (DChart.Methods.JudgeBetweenAngle(Math.PI, Math.PI * 2, computeinfo.angleMax) && !preventinnerLine) {
        ...
        inner.DrawFigures.createCloseFigure = function (points, fillcolor, linewidth, linecolor, smoothline, upturnAxis, shadow) {
            var ctx = inner.ctx;
            ctx.save();
            ctx.beginPath();
            var len = points.length;
        ...
        inner.DrawFigures.create3DHistogram = function (left, top, width, height, fillcolor, linewidth, angle, longlength, linecolor, topdarksidecolor, rightdarksidecolor, shadow) {
            topdarksidecolor = topdarksidecolor || DChart.Methods.getDarkenColor(fillcolor);
