if (!window.DChart) {
    ...
    DChart.Const.Skins.BlackAndWhite.MultiRing3D = {
...
DChart.MultiRing3D = DChart.getCore().__extends({ GraphType: 'MultiRing3D' });
DChart.MultiRing3D._spreadSkin = function (newOps, skin) {
...
DChart.MultiRing3D._getDefaultOptions = function (originalCommonOptions) {
...
DChart.MultiRing3D._getCheckOptions = function () {
    return {
...
DChart.MultiRing3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var radius = !options.radius || !DChart.Methods.IsNumber(options.radius) ? radiusInfo.maxRadius : options.radius;
    ...
    var mouseEvents = function () {
        ...
        var fixShape = function (x, y) {
            ...
            for (var i = 0, shape; shape = shapes.cemicircles[i]; i++) {
                ...
                if (distance2 >= Math.pow(computeinfo.innerRadius, 2) && distance2 <= Math.pow(computeinfo.outerRadius, 2) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentAngle)) {
            ...
            if (!veryShape) {
                for (var i = shapes.cemicircles.length - 1; i >= 0; i--) {
                    ...
                    if (distance2 >= Math.pow(computeinfo.innerRadius, 2) && distance2 <= Math.pow(computeinfo.outerRadius, 2) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentDarkAngle)) {
                        veryShape = shape; break;
                    }
                }
            }
