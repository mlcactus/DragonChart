if (!window.DChart) {
    ...
    DChart.Const.Skins.BlackAndWhite.QueueHistogram3D = {
        LabelColor: null
    };
}
DChart.QueueHistogram3D = DChart.getCore().__extends({ GraphType: 'QueueHistogram3D' });
DChart.QueueHistogram3D._spreadSkin = function (newOps, skin) {
    newOps.label = {};
    newOps.label.color = skin.LabelColor || null;
};
DChart.QueueHistogram3D._getDefaultOptions = function (originalCommonOptions) {
...
DChart.QueueHistogram3D._getCheckOptions = function () {
    return {
...
DChart.QueueHistogram3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var drawsplitpoint = function () {
        ...
        if (options.splitLine.show && linewidth && linewidth > 0) {
            ...
            inner.DrawFigures.create3DHistogram(left, top, axisSize.maxX - axisSize.minX, linewidth, linecolor, 0, axisSize.sightangle, stagewidth, null, DChart.Methods.getDarkenColor(linecolor, 0.7), DChart.Methods.getDarkenColor(linecolor, 0.8));
        }
    };
    var drawPart = function (shape, color, mouseon, drawgraphic, drawtext) {
        ...
        if (drawgraphic) {
            inner.DrawFigures.create3DHistogram(_left, _top, shape.width, shape.height, _color, 0, axisSize.sightangle, axisSize.stagewidth, null, mouseon ? color : shape.topcolor || DChart.Methods.getDarkenColor(_color, 0.7), mouseon ? color : shape.rightcolor || DChart.Methods.getDarkenColor(_color, 0.8));
        }
        if (drawtext && options.label.show) {
            ...
            if (content) {
                ...
                var size = ops.fontsize || DChart.Methods.CapValue(shape.width / 2, 18, 11);
