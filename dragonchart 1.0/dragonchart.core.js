...
DChart.getCore = function () {
    var core = function (_targetdiv, _language) {
        ...
        inner._computeAxis = function (valids) {
            ...
            var labelDistance = (invertAxis ? maxY - minY : maxX - minX) / (labelCount + (fromFirstLeft ? -1 : (multiple ? 1 / (inner.innerData.length + 1) : 1 / 3)));
            var startPos = (invertAxis ? minY : minX) + (fromFirstLeft ? 0 : labelDistance * (multiple ? (inner.innerData.length / 2 + 1) / (inner.innerData.length + 1) : 2 / 3));
        ...
        inner._createAxis = function (valids) {
            ...
            var vfontsize = options.valueAxis.fontsize || (invertAxis ? 1.3 : 1) * (axisSize.valueAxisLength - axisSize.valueAxisLineWidth) / (vTimeType ? 9 : 6);
            ...
            var drawXAxisTitle = function () {
                ...
                var centerX = inner.canvas.width / 2;
                var bottom = axisSize.maxY + axisSize.xAxisLineWidth + (axisSize.xAxisHeight - axisSize.xAxisLineWidth) * (options.xAxisTitle.titlelocation || (invertAxis ? 0.75 : DChart.Const.Defaults.AxisXTitleLocation[axisData.vValueType]));
