    ...
    var core = function (_targetdiv, _language) {
        ...
        inner.SavePic = function (filename, type) {
            ...
            var saveFile = function (data, filename) {
                var save_link = document.createElementNS('http:                save_link.href = data;
...
(function () {
    function loadScript(url) {
        ...
        for (var i = 0, _script; _script = allScript[i]; i++) {
            if (_script.src.indexOf('dragonchart.all.') >= 0) {
                allScript = _script; break;
            }
        }
        try {
            var url = allScript.src.replace('dragonchart.all.', url);
