...
DChart.MultiRing = DChart.getCore().__extends({
    ...
    Draw: function (_data, ops) {
        ...
        var radius = options.radius && options.radius < maxRadius ? options.radius : maxRadius;
        ...
        inner.coordinates.multiRing = { radius: radius, centerX: coordinate.centerX, centerY: coordinate.centerY };
            ...
            if (options.tip.show && typeof options.tip.content == 'function') {
                ...
                this.showTip = function () {
                    if (this.tip) {
                        ...
                        var left = coordinate.centerX + (this.outerRadius + this.innerRadius) / 2 * Math.cos(midAngle);
                        var top = coordinate.centerY + (this.outerRadius + this.innerRadius) / 2 * Math.sin(midAngle);
        ...
        var drawPart = function (scalePercent, innerRadius, outerRadius, angleMin, angleMax, color, drawOuterLabel, data, pieshape) {
            ...
            ctx.save();
            ctx.beginPath();
            ctx.moveTo(centerX + outerRadius * scalePercent * Math.cos(angleMin), centerY + outerRadius * scalePercent * Math.sin(angleMin));
            ctx.arc(centerX, centerY, outerRadius * scalePercent, angleMin, angleMax);
            ctx.lineTo(centerX + innerRadius * scalePercent * Math.cos(angleMax), centerY + innerRadius * scalePercent * Math.sin(angleMax));
            ctx.arc(centerX, centerY, innerRadius * scalePercent, angleMax, angleMin, true);
            ctx.closePath();
            ctx.fillStyle = color;
            ctx.fill();
            ctx.restore();
        ...
        var drawSegments = function (animationDecimal, percentAnimComplete) {
            ...
            for (var k = 0; k < ringCount; k++) {
                ...
                for (var i = 0, item; item = inner.innerData[i]; i++) {
                    var tmpVal = item.value[k];
                    var percent = (tmpVal / segmentTotal[k]) * 100;
        ...
        var mouseEvents = function () {
            ...
            var fixShape = function (x, y) {
                ...
                for (var i = 0, shape; shape = inner.shapes.cemicircles[i]; i++) {
                    var midAngle = (shape.angleMin + shape.angleMax) / 2;
                    var currentAngle = DChart.Methods.GetCurrentAngle(x, y, coordinate.centerX, coordinate.centerY);
                    var distance = Math.pow(x - coordinate.centerX, 2) + Math.pow(y - coordinate.centerY, 2);
                    if (distance <= Math.pow(shape.outerRadius + separeateLineWidth, 2) && distance >= Math.pow(shape.innerRadius - separeateLineWidth, 2) && DChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentAngle)) {
                        veryShape = shape;
                        break;
                    }
                }
